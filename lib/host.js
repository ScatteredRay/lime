...
const app = new(function() {
    ...
    function LoadAppFile(mod) {
        ...
        function readFile() {
            ...
            function onProcessedData (data) {
                console.debug("onProcessedData", file);
                ...
                function pushPriority(module) {
                    for(var i in module.depends) {
                        ...
                        if(typeof appList[dep].object === 'undefined') {
                            appList[dep].object = {};
                            sync.addObject(appList[dep].object, dep)
                        }
                        ...
                        console.debug("Adding " + module.name + " as dependant to " + dep);
                ...
                // TODO: It seems we can get into a case where we get here and send an object when it's dependents aren't loaded. We should try and handle that.
                sync.dirtyObject(exports, mod);
            }

            console.debug("readFile", file);
    ...
    function ReadAppManifest(manifest) {
        function readAppFile() {
            console.info("Loading app manifest file: " + manifest);
            fs.readFile(
                manifest,
                function(err, data) {
                    if(err) {
